...
async def send_message_to_agent(
    user: str, whatsapp_config: WhatsAppConfig, session_id: str, message: str
) -> str:
    """Sends a message to the internal agent service and parses the response."""
    logger = get_logger(
        "agent_communication", {"app_name": whatsapp_config.agent_app_name}
    )
...
async def _send_whatsapp_acknowledgment(
    user_wa_id: str, message_text: str, whatsapp_config: WhatsAppConfig
) -> bool:
    ...
    try:
        ...
        await send_whatsapp_message(
            user_wa_id, message, f"{whatsapp_config.api_url}/messages", whatsapp_config.token
        )
...
async def _process_webhook_in_background(body: dict, app_type: AppType) -> None:
    ...
    for sender_wa_id, message in webhook_payload.get_all_messages():
        try:
            await process_message(sender_wa_id, message, whatsapp_config)
        except Exception as e:
            logging.error(f"Error processing message {message.id}: {e}", exc_info=True)
            await _send_whatsapp_acknowledgment(
                sender_wa_id, "Error procesando mensaje.", whatsapp_config
            )
...
async def process_message(
    sender_wa_id: str, message: "WhatsAppMessage", whatsapp_config: WhatsAppConfig
) -> None:
...
async def _process_single_text_message(
    ...
    agent_response = await send_message_to_agent(
        sender_wa_id, whatsapp_config, sender_wa_id, message_text
    )
...
async def handle_audio_message(
    phone: str, audio_id: str, whatsapp_config: WhatsAppConfig
) -> None:
    ...
    if not api_url or not token:
        logging.error(
            f"Incomplete WhatsApp config for audio in {whatsapp_config.app_type.value}"
        )
        return
